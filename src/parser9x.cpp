...
void Parser9x::processSpecialCharacter(wvWare::UChar character, U32 globalCP, SharedPtr<const Word97::CHP> chp )
{
...
void Parser9x::processFootnote(wvWare::UChar character, U32 globalCP, SharedPtr<const Word97::CHP> chp )
{
    if ( !m_footnotes ) {
        ...
        int totalPicfSize, int picfStartPos )
{
    ...
    do
    {
		...
		if (header.getFbt() == 0xF007)
		{
			...
			if (h.getFbt() < 0xF018 || h.getFbt() > 0xF117)
			{
				wvlog << "Error! EscherHeader is not a msofbtBlip!" << std::endl;
				stream->seek(h.recordSize(), G_SEEK_CUR);
			...
			if (blip.isCompressed())
			{
				...
				m_pictureHandler->escherData(outBuffer, data.picf, h.getFbt() - 0xF018);
			}
			...
			else
			{
				...
				m_pictureHandler->escherData(reader, data.picf, h.getFbt() - 0xF018);
		...
		else
		{
			...
			stream->seek(header.recordSize(), G_SEEK_CUR);
