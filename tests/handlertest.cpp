...
{
public:
    void bitmapData( wvWare::OLEImageReader& reader, wvWare::SharedPtr<const wvWare::Word97::PICF> picf ) override;
    void wmfData( wvWare::OLEImageReader& reader, wvWare::SharedPtr<const wvWare::Word97::PICF> picf ) override;
    void externalImage( const wvWare::UString& name, wvWare::SharedPtr<const wvWare::Word97::PICF> picf ) override;
	void escherData(wvWare::OLEImageReader& reader, wvWare::SharedPtr<const wvWare::Word97::PICF> picf, int type) override;
	void escherData(std::vector<wvWare::U8> data, wvWare::SharedPtr<const wvWare::Word97::PICF> picf, int type) override;
};
...
void PictureTest::escherData(wvWare::OLEImageReader& reader, wvWare::SharedPtr<const wvWare::Word97::PICF> picf, int type)
{
	if (!reader.isValid())
		return;

	size_t imageSize = reader.size();
	std::vector<wvWare::U8> imgData(imageSize);

	reader.read(imgData.data(), imageSize);

	std::ofstream file("d:\\TEST_F\\doc_test\\file.png", std::ios::binary);
	file.write((char*)imgData.data(), imageSize);
}


void PictureTest::escherData(std::vector<wvWare::U8> imgData, wvWare::SharedPtr<const wvWare::Word97::PICF> picf, int type)
{
	std::ofstream file("d:\\TEST_F\\doc_test\\file.wmf", std::ios::binary);
	file.write((char*)imgData.data(), imgData.size());
}
...
void TextTest::footnoteFound( FootnoteData::Type type, wvWare::UChar character,
...
int main( int argc, char** argv )
{
    ...
    std::string document( "d:\\TEST_F\\doc_stamps.doc" );
	...
	parser->setPictureHandler(pictureTest);
